	...
	factory: (require) => {
		...
		const LOCALES = {
			zh: {
				...
				modelsUnit: '个模型',
				keyMissing: '未配 Key',
				noBaseURLHint: '未配置 baseURL，无法探测；模型为手填目录',
				pullCatalog: '拉取目录',
				pulling: '拉取中…',
				copyIds: '复制全部 id',
				copied: '已复制 id',
			...
			en: {
				...
				modelsUnit: 'models',
				keyMissing: 'No key',
				noBaseURLHint: 'No baseURL configured; probing unavailable, models are hand-curated',
		 ...
		 * 适配器给出的 reason（如"当前用户不存在coding plan"）原样透传 —— 那是
		 * 上游的人话，比"查询失败"有用。
		...
		function PiAiCard(props) {
			...
			if (status === null || status.ok !== true) {
				return h('div', { className: 'dsh-llm-hub-balance' },
					...
					h('span', { className: 'dsh-llm-hub-balance__breakdown' }, status !== null && typeof status.error === 'string' ? status.error : t('statusFail')))
			}
			...
			// modelgo 走 anthropic-messages，官方「获取可用模型」对它天然失效 ——
			// 这里额外提供旁路目录拉取与一键复制 id；其余 provider 保持轻量。
			if (provider !== 'modelgo') return primary
			...
			return h('div', null, primary, h('div', { className: 'dsh-llm-hub-balance' }, extra))
		}
