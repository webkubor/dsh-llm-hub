...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 路由组状态：模式、激活组、每组候选与可用性、最近实际路由。 */
		const ROUTING_STATE_URL = '/api/dsh-llm-hub/routing/state'
		/** 路由组切换：POST { mode?, activeGroup? }，持久化进 settings。 */
		const ROUTING_SELECT_URL = '/api/dsh-llm-hub/routing/select'
		/** 模型别名：把长 id 显示成短名（人话）。 */
		const ALIASES_URL = '/api/dsh-llm-hub/aliases'
		const BILLING_MODE_URL = '/api/dsh-llm-hub/billing-mode'
		...
		const LOCALES = {
			zh: {
				...
				billingUsage: '按量付费',
				billingSub: '套餐',
				billingSwitchToSub: '切换为套餐模式',
				billingSwitchToUsage: '切换为按量付费模式',
				...
				routeChipSmart: '智能',
				routeChipManual: '手动',
				routeChipModeSmart: '智能路由',
				routeChipModeManual: '手动模型',
				routeChipWouldRoute: '将路由到',
				routeChipLastRoute: '最近实际',
				routeChipReasonNone: '候选全部不可用',
				routeChipFailOpen: '路由失败时按原配置发出',
			...
			en: {
				...
				billingUsage: 'Pay-As-You-Go',
				billingSub: 'Plan',
				billingSwitchToSub: 'Switch to Plan mode',
				billingSwitchToUsage: 'Switch to Pay-As-You-Go mode',
				granted: 'Granted',
				toppedUp: 'Topped up',
				...
				routeChipSmart: 'Smart',
				routeChipManual: 'Manual',
				routeChipModeSmart: 'Smart routing',
				routeChipModeManual: 'Manual model',
				routeChipWouldRoute: 'Routes to',
				routeChipLastRoute: 'Last used',
				routeChipReasonNone: 'All candidates unavailable',
				routeChipFailOpen: 'Falls back to your selection when routing fails',
		...
		function ensureStyle() {
			...
			style.textContent = [
				...
				'.dsh-llm-hub-billing-capsule{display:inline-flex;align-items:center;background:var(--dsw-alias-fill-quaternary, rgba(125,125,125,0.08));border-radius:12px;padding:1.5px;gap:2px;user-select:none;margin-right:2px}',
				'.dsh-llm-hub-billing-pill{font-size:10px;line-height:14px;padding:1px 6px;border-radius:10px;cursor:pointer;color:var(--dsw-alias-label-tertiary);transition:all .15s ease}',
				'.dsh-llm-hub-billing-pill:hover{color:var(--dsw-alias-label-primary)}',
				'.dsh-llm-hub-billing-pill.is-active{font-weight:600;background:var(--dsw-alias-fill-primary, #fff);box-shadow:0 1px 2px rgba(0,0,0,0.08)}',
				'.dsh-llm-hub-billing-pill.is-active.is-usage{color:#2563eb}',
				'.dsh-llm-hub-billing-pill.is-active.is-sub{color:#059669}',
				'.dsh-llm-hub-key-chip{display:inline-flex;align-items:center;gap:3px;font-size:10.5px;padding:1px 5px;border-radius:4px;background:var(--dsw-alias-fill-quaternary, rgba(125,125,125,0.06));border:1px solid var(--dsw-alias-line-secondary, rgba(125,125,125,0.15));margin-right:4px}',
				'.dsh-llm-hub-key-chip.is-down{opacity:.6;border-style:dashed}',
				// 去掉粗糙灰底框，改用轻量排版标签
				...
				// composer 快切 chip（conversation.input.left）
				'.dsh-llm-hub-route{position:relative;display:inline-flex}',
				'.dsh-llm-hub-route__btn{box-sizing:border-box;display:inline-flex;align-items:center;gap:4px;height:22px;',
				'padding:0 8px;border-radius:6px;border:.5px solid var(--dsw-alias-border-l1);background:transparent;cursor:pointer;',
				'font:inherit;font-size:11px;color:var(--dsw-alias-label-secondary);transition:all .15s ease;white-space:nowrap}',
				'.dsh-llm-hub-route__btn:hover{border-color:var(--dsw-alias-border-l2);color:var(--dsw-alias-label-primary);',
				'background:var(--dsw-alias-bg-layer-2)}',
				'.dsh-llm-hub-route__btn.is-smart{border-color:var(--dsw-alias-state-warn-primary);color:var(--dsw-alias-state-warn-primary)}',
				'.dsh-llm-hub-route__btn.is-busy{opacity:.55;cursor:progress}',
				'.dsh-llm-hub-route__mark{font-size:11px;line-height:1}',
				'.dsh-llm-hub-route__menu{position:absolute;bottom:calc(100% + 6px);left:0;z-index:60;min-width:240px;',
				'padding:8px;border-radius:8px;border:.5px solid var(--dsw-alias-border-l2);',
				'background:var(--dsw-alias-bg-layer-1,rgba(15,15,15,.96));backdrop-filter:blur(14px);',
				'box-shadow:0 8px 24px rgba(0,0,0,.28);display:flex;flex-direction:column;gap:8px;font-size:11px}',
				'.dsh-llm-hub-route__modes{display:flex;gap:4px}',
				'.dsh-llm-hub-route__mode{flex:1;padding:4px 6px;border-radius:5px;border:.5px solid var(--dsw-alias-border-l1);',
				'background:transparent;color:var(--dsw-alias-label-secondary);cursor:pointer;font:inherit;font-size:10.5px;transition:all .15s ease}',
				'.dsh-llm-hub-route__mode.is-on{border-color:var(--dsw-alias-state-warn-primary);color:var(--dsw-alias-state-warn-primary);',
				'background:var(--dsw-alias-bg-layer-2)}',
				'.dsh-llm-hub-route__group{display:flex;flex-direction:column;gap:3px;padding:6px 7px;border-radius:6px;',
				'border:.5px solid var(--dsw-alias-border-l1);background:transparent;cursor:pointer;text-align:left;',
				'font:inherit;color:var(--dsw-alias-label-primary);transition:all .15s ease}',
				'.dsh-llm-hub-route__group:hover{border-color:var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2)}',
				'.dsh-llm-hub-route__group.is-active{border-color:var(--dsw-alias-state-warn-primary)}',
				'.dsh-llm-hub-route__group-top{display:flex;align-items:center;justify-content:space-between;gap:6px}',
				'.dsh-llm-hub-route__group-name{font-weight:600;font-size:11px}',
				'.dsh-llm-hub-route__state{font-size:10px;color:var(--dsw-alias-label-secondary);max-width:150px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}',
				'.dsh-llm-hub-route__hint{font-size:10px;color:var(--dsw-alias-label-tertiary)}',
				// Hero 全景统计大横条
				...
				// 订阅制配额可视化进度条与重置倒计时（参考 claude-usage-statusline 规范）
				'.dsh-llm-hub-quota-group{display:inline-flex;align-items:center;gap:6px;flex-wrap:wrap;vertical-align:middle}',
				'.dsh-llm-hub-quota-bar{display:inline-flex;align-items:center;gap:4.5px;padding:1.5px 6px;border-radius:4px;background:var(--dsw-alias-bg-layer-2,rgba(255,255,255,.03));border:.5px solid var(--dsw-alias-border-l1,rgba(255,255,255,.08));font-size:10.5px;line-height:1;transition:all .15s ease}',
				'.dsh-llm-hub-quota-bar:hover{border-color:var(--dsw-alias-brand-primary,rgba(59,130,246,.4));background:var(--dsw-alias-bg-layer-3,rgba(255,255,255,.06))}',
				'.dsh-llm-hub-quota-label{font-weight:600;color:var(--dsw-alias-label-primary);font-size:10px}',
				'.dsh-llm-hub-quota-track{width:46px;height:4.5px;border-radius:2.5px;background:rgba(255,255,255,.12);overflow:hidden;position:relative;display:inline-block;vertical-align:middle}',
				'.dsh-llm-hub-quota-fill{height:100%;border-radius:2.5px;display:block;transition:width .3s cubic-bezier(0.4,0,0.2,1)}',
				'.dsh-llm-hub-quota-fill.is-high{background:linear-gradient(90deg,#10b981,#34d399)}',
				'.dsh-llm-hub-quota-fill.is-mid{background:linear-gradient(90deg,#f59e0b,#fbbf24)}',
				'.dsh-llm-hub-quota-fill.is-low{background:linear-gradient(90deg,#ef4444,#f87171)}',
				'.dsh-llm-hub-quota-pct{font-variant-numeric:tabular-nums;font-weight:600;color:var(--dsw-alias-label-primary);font-size:10.5px}',
				'.dsh-llm-hub-quota-time{font-size:9.5px;color:var(--dsw-alias-label-secondary);opacity:.8;font-variant-numeric:tabular-nums;display:inline-flex;align-items:center;margin-left:1px}',
				// Webkubor 插件全家桶互导矩阵
				...
				'.dsh-suite-badge-unknown{display:inline-flex;align-items:center;gap:4px;color:var(--dsw-alias-label-tertiary);font-weight:550;font-size:10.5px}',
		...
		function BalanceCard(props) {
			...
			const verdict = Array.isArray(availabilityState.providers)
				...
				: undefined
			const hiddenFromDropdown = Array.isArray(availabilityState.hidden)
				? availabilityState.hidden.includes(DEEPSEEK_PROVIDER)
				...
				fetch(BALANCE_URL, { headers: { accept: 'application/json' } })
					.then(async (response) => {
						...
						setState((prev) => ({
							...
							billingMode: typeof body.billingMode === 'string' ? body.billingMode : (prev.billingMode || 'usage')
						}))
					})
					.catch((error) => {
			...
			const mode = state.billingMode || 'usage'
			const switchMode = (newMode) => {
				setState((prev) => ({ ...prev, billingMode: newMode }))
				fetch(BILLING_MODE_URL, {
					method: 'POST',
					headers: { 'content-type': 'application/json' },
					body: JSON.stringify({ provider: 'deepseek', mode: newMode })
				}).catch(() => {})
			}
			...
			return h('div', { className: 'dsh-llm-hub-card' },
				h('div', { className: 'dsh-llm-hub-card__meta' },
					h('div', { className: 'dsh-llm-hub-card__tags' },
						h('span', { className: 'dsh-llm-hub-billing-capsule' },
							h('span', {
								className: 'dsh-llm-hub-billing-pill ' + (mode === 'usage' ? 'is-active is-usage' : ''),
								title: mode === 'usage' ? t('billingSwitchToSub') : t('billingSwitchToUsage'),
								onClick: () => switchMode('usage')
							}, t('billingUsage')),
							h('span', {
								className: 'dsh-llm-hub-billing-pill ' + (mode === 'subscription' ? 'is-active is-sub' : ''),
								title: mode === 'subscription' ? t('billingSwitchToUsage') : t('billingSwitchToSub'),
				...
				state.keys && state.keys.length > 1
					? h('div', { className: 'dsh-llm-hub-card__actions', style: { paddingTop: '3px' } },
						h('div', { className: 'dsh-llm-hub-card__tags' },
							state.keys.map((k, idx) => h('span', {
								key: k.ref,
								className: 'dsh-llm-hub-key-chip' + (k.isAvailable ? '' : ' is-down'),
								title: `${k.ref}: ${k.isAvailable ? '正常' : (k.error || '不可用')}`
							}, `Key ${idx + 1} (${k.keyMasked}): ${k.isAvailable ? (k.currency === 'USD' ? '$' : '¥') + k.total : '⚠️ 余额不足'}`))
						)
					)
					: null
			)
		...
		function detectBillingMode(profile, status, balance) {
		...
		function renderBalance(balance, t) {
			...
			const isUsed = balance.meaning === 'used'
			const bars = []
			for (const item of items.slice(0, 2)) {
				const remainPct = isUsed ? Math.max(0, 100 - (item.percent ?? 0)) : (item.percent ?? 0)
				const fillPct = Math.min(Math.max(remainPct, 0), 100)
				const levelClass = fillPct > 50 ? 'is-high' : (fillPct > 20 ? 'is-mid' : 'is-low')
				const labelText = item.label || '5h'
				const resetTimeNote = item.resetsAt ? `(${item.resetsAt} 重置)` : ''
				bars.push(h('span', {
					key: labelText,
					className: 'dsh-llm-hub-quota-bar',
					title: `${labelText} ${meaning} ${item.percent}%${resetTimeNote ? ` · ${resetTimeNote}` : ''}`
				},
					h('span', { className: 'dsh-llm-hub-quota-label' }, labelText),
					h('span', { className: 'dsh-llm-hub-quota-track' },
						h('span', { className: `dsh-llm-hub-quota-fill ${levelClass}`, style: { width: `${fillPct}%` } })
					),
					h('span', { className: 'dsh-llm-hub-quota-pct' }, `${item.percent}%`),
					resetTimeNote ? h('span', { className: 'dsh-llm-hub-quota-time' }, resetTimeNote) : null
				))
				if (typeof item.weeklyPercent === 'number') {
					const weeklyRemain = isUsed ? Math.max(0, 100 - item.weeklyPercent) : item.weeklyPercent
					const weeklyFill = Math.min(Math.max(weeklyRemain, 0), 100)
					const weeklyClass = weeklyFill > 50 ? 'is-high' : (weeklyFill > 20 ? 'is-mid' : 'is-low')
					bars.push(h('span', {
						key: 'week',
						className: 'dsh-llm-hub-quota-bar',
						title: `${t('week')} ${meaning} ${item.weeklyPercent}%`
					},
						h('span', { className: 'dsh-llm-hub-quota-label' }, t('week')),
						h('span', { className: 'dsh-llm-hub-quota-track' },
							h('span', { className: `dsh-llm-hub-quota-fill ${weeklyClass}`, style: { width: `${weeklyFill}%` } })
						),
						h('span', { className: 'dsh-llm-hub-quota-pct' }, `${item.weeklyPercent}%`)
					))
				}
			}
			return h('span', { className: 'dsh-llm-hub-quota-group' }, bars)
		}
		...
		function PiAiCard(props) {
			...
			const [warning, setWarning] = React.useState(null)
			const [billingModeOverride, setBillingModeOverride] = React.useState(null)
			...
			const verdict = Array.isArray(availabilityState.providers)
				...
				: undefined
			const hiddenFromDropdown = Array.isArray(availabilityState.hidden)
				? availabilityState.hidden.includes(provider)
				: (verdict !== undefined && verdict.state === 'unavailable')
			...
			const currentBillingMode = billingModeOverride || status.billingMode || detectBillingMode(props.provider, status, balance)
			const switchBillingMode = (newMode) => {
				setBillingModeOverride(newMode)
				fetch(BILLING_MODE_URL, {
					method: 'POST',
					headers: { 'content-type': 'application/json' },
					body: JSON.stringify({ provider, mode: newMode })
				}).catch(() => {})
			}

			// 元信息标签组（不重复展示外层已有的大标题，只呈现核心事实）
			const tags = []
			tags.push(h('span', { key: 'billing-capsule', className: 'dsh-llm-hub-billing-capsule' },
				h('span', {
					className: 'dsh-llm-hub-billing-pill ' + (currentBillingMode === 'usage' ? 'is-active is-usage' : ''),
					title: currentBillingMode === 'usage' ? t('billingSwitchToSub') : t('billingSwitchToUsage'),
					onClick: () => switchBillingMode('usage')
				}, t('billingUsage')),
				h('span', {
					className: 'dsh-llm-hub-billing-pill ' + (currentBillingMode === 'subscription' ? 'is-active is-sub' : ''),
					title: currentBillingMode === 'subscription' ? t('billingSwitchToUsage') : t('billingSwitchToSub'),
					onClick: () => switchBillingMode('subscription')
				}, t('billingSub'))
			))
		...
		/**
		 * composer 路由快切 chip（1.5.0 智能路由组）。
		 *
		 * 挂在 `conversation.input.left`（composer 工具行左端，追加型 list slot）：
		 *   · 未配置任何路由组时整个 chip 不渲染 —— 存在即有意义；
		 *   · 关起时一行小字：⚡智能 · 公司（smart 模式）/ 🧭 · 公司（manual）；
		 *   · 点开是浮层：模式二选一（智能路由 = 接管，实际路由按额度自动切；
		 *     手动 = 尊重你在模型下拉里选的），下面是组列表（每组显示「将路由到
		 *     哪个」与可用状态），点组即切、立即持久化。
		 *
		 * 数据流：GET /routing/state（挂载 + 每 30s 轮询 + 操作后重拉），
		 * 写操作 POST /routing/select。chip 显示「最近实际路由」—— smart 模式
		 * 下模型下拉显示什么已经不重要，这里才是事实。
		 */
		function SmartRouteChip(props) {
			const t = typeof props.t === 'function' ? props.t : fallbackT
			const [state, setState] = React.useState(null)
			const [open, setOpen] = React.useState(false)
			const [busy, setBusy] = React.useState(false)
			const query = React.useCallback(() => {
				fetch(ROUTING_STATE_URL, { headers: { accept: 'application/json' } })
					.then((res) => res.json())
					.then((body) => { if (body !== null && typeof body === 'object' && body.ok === true) setState(body) })
					.catch(() => {})
			}, [])
			React.useEffect(() => {
				query()
			...
			// 一个组都没有：chip 无事可做，不渲染（配置在 settings.yaml 里加）。
			if (state === null || !Array.isArray(state.groups) || state.groups.length === 0) return null
			ensureStyle()
			const activeGroup = state.groups.find((group) => group.id === state.activeGroup) ?? state.groups[0]
			const isSmart = state.mode === 'smart'
			const chipClass = [
				'dsh-llm-hub-route__btn',
				isSmart ? 'is-smart' : '',
				busy ? 'is-busy' : ''
			].filter(Boolean).join(' ')
			const label = (activeGroup && (activeGroup.label ?? activeGroup.id)) ?? ''
			const chipTitle = isSmart
				? `${t('routeChipModeSmart')} · ${label} —— ${state.lastDecision?.to ? `${t('routeChipLastRoute')}: ${state.lastDecision.to.provider}/${state.lastDecision.to.model}` : t('routeChipFailOpen')}`
				: `${t('routeChipModeManual')} · ${label}`
			const select = async (patch) => {
				setBusy(true)
				fetch(ROUTING_SELECT_URL, {
					method: 'POST',
					headers: { 'content-type': 'application/json', accept: 'application/json' },
					...
					.then((body) => { if (body !== null && typeof body === 'object' && body.ok === true) setState(body) })
					.catch(() => {})
					.finally(() => { setBusy(false); query() })
			}
			const stateText = (group) => {
				if (group.wouldRoute) {
					const next = `${group.wouldRoute.provider}/${group.wouldRoute.model}`
					return group.reason !== null && group.reason !== undefined ? group.reason : `${t('routeChipWouldRoute')}: ${next}`
				}
				return group.reason ?? t('routeChipReasonNone')
			}
			const menu = open ? h('div', { className: 'dsh-llm-hub-route__menu' },
				h('div', { className: 'dsh-llm-hub-route__modes' },
					h('button', {
						type: 'button',
						className: `dsh-llm-hub-route__mode${isSmart ? ' is-on' : ''}`,
						onClick: () => { if (!isSmart) select({ mode: 'smart' }) }
					}, `⚡ ${t('routeChipModeSmart')}`),
					h('button', {
						type: 'button',
						className: `dsh-llm-hub-route__mode${!isSmart ? ' is-on' : ''}`,
						onClick: () => { if (isSmart) select({ mode: 'manual' }) }
					}, t('routeChipModeManual'))),
				...state.groups.map((group) => h('button', {
					key: group.id,
					type: 'button',
					className: `dsh-llm-hub-route__group${group.id === state.activeGroup ? ' is-active' : ''}`,
					onClick: () => {
						const patch = { activeGroup: group.id }
						if (!isSmart) patch.mode = 'smart'
						select(patch)
					}
				},
					h('span', { className: 'dsh-llm-hub-route__group-top' },
						h('span', { className: 'dsh-llm-hub-route__group-name' }, group.label ?? group.id),
						h('span', { className: 'dsh-llm-hub-route__state' }, stateText(group)))))) : null
			return h('div', { className: 'dsh-llm-hub-route' },
				h('button', {
					type: 'button',
					className: chipClass,
					title: chipTitle,
					onClick: () => { setOpen(!open); query() }
				},
					h('span', { className: 'dsh-llm-hub-route__mark' }, isSmart ? '⚡' : '🧭'),
					h('span', null, `${isSmart ? `${t('routeChipSmart')} · ` : ''}${label}`)),
				menu)
		}
		...
		function HubFooter(props) {
			...
			links.push(h('button', {
				...
				onClick: () => {
					...
					// 第 2 步对 `dsh plugin add` 是自动的（装完会 reconcile bundles），
					// 只有手写 package.json 或从别处搬时才需要手动补，所以标成「如果没自动加」。
					const text = [
						`${meta.name} —— DSH 模型页增强：网关可达性探测、余额/配额、模型目录勾选写回配置。`,
						'装到你的 DSH（profile 名按自己的改，默认 web）：',
						`  1) dsh plugin --profile web add ${meta.name}`,
						`  2) 确认 "${meta.name}" 在 ~/.dsh/profiles/web/package.json 的 dsh.profile.bundles 里（dsh plugin add 会自动加）`,
						'  3) 重启 DSH',
		...
		/**
		 * 已激活的客户端插件 id 集合 —— 读宿主注入的 `window.__DSH_BOOT__.entries`。
		 *
		 * 为什么是这个源：`__DSH_BOOT__` 由 `dsh-client-modules` 在渲染 index 时注入，
		 * 就是浏览器真正要拉取并执行的那份组合图。它在，插件就一定活着；它不在，
		 * 无论 npm 装没装都不会加载。比「读 profile 的 package.json」更硬：后者只说明
		 * 装过，**接没接进 boot graph 是另一回事**（少了 `dsh.profile.bundles` 那行，
		 * 依赖装了也不加载），而那正是这套 Dock 想告诉人的事。
		 *
		 * 读不到就返回 null（不是空集合）：分不清「一个都没装」和「读不到」时，
		 * 宁可不做判定 —— 全部显示成未安装，是把探测失败谎报成事实。
		 * @returns 已激活的 id 集合，或 null。
		 */
		function readActivatedPluginIds() {
			try {
				const boot = typeof window === 'undefined' ? undefined : window.__DSH_BOOT__
				if (boot === null || typeof boot !== 'object' || !Array.isArray(boot.entries)) return null
				return new Set(boot.entries.filter((entry) => entry && typeof entry.id === 'string').map((entry) => entry.id))
			} catch {
				return null
			}
		}

		/** 一个家族条目是否已激活：按 id 与 aliases 逐个比对（scope 迁移过的包名都在 aliases 里）。 */
		function isSuiteItemActive(item, activated) {
			if (activated === null) return null
			for (const candidate of [item.id, ...(item.aliases ?? [])]) {
				if (activated.has(candidate)) return true
			}
			return false
		}
		...
		function WebkuborSuiteDock(props) {
			...
			// 判定在 render 里现读：`__DSH_BOOT__` 是宿主渲染 index 时注入的全局，之后
			// 不会再变（组合图变了要整页重载），所以不需要状态也不需要订阅。
			const activated = readActivatedPluginIds()

			const handleCopy = (pkg) => {
				// 命令必须带 `--profile`：`dsh plugin` 是 pnpm 的透传层，profile 是
				// **必填**选项，漏了直接报错退出 —— 复制出去的命令得是能跑的。
				const cmd = `dsh plugin --profile web add ${pkg}`
			...
			return h('div', { className: 'dsh-suite-dock' },
				...
				h('div', { className: 'dsh-suite-cards' },
					...suite.map((item) => {
						const active = isSuiteItemActive(item, activated)
						...
						return h('div', { key: item.id, className: 'dsh-suite-card' },
							...
							h('div', { className: 'dsh-suite-card-bottom' },
								active === true ? (
									...
									// 读不到 boot graph（老宿主 / 非 web 载体）：说「判定不了」，
									// 不说「没装」—— 后者是把探测失败当结论。
									h('span', { className: 'dsh-suite-badge-unknown' }, t('suiteUnknown'))
								) : (
									h('button', {
										type: 'button',
										className: 'dsh-suite-btn-action',
										onClick: () => handleCopy(item.pkg),
										title: `安装命令: dsh plugin --profile web add ${item.pkg}`
		...
		function apply(ctx) {
			...
			// composer 工具行左端的路由快切 chip —— 「切公司 / 切个人 + 智能开关」
			// 的高频操作留在聊天页，不用进设置页。list slot 必须带 id（同 footer 的教训）。
			ctx.slots.inject('conversation.input.left', () => ctx.slots.register({
				name: 'conversation.input.left',
				id: 'dsh-llm-hub-route-chip',
				order: 20,
				locale: NS,
				inject: () => ({ t })
			}, SmartRouteChip))
