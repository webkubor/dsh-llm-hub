...
export function apply(ctx) {
	...
	const installProviderFilter = () => {
		...
		const filtered = function (...args) {
			...
			const available = hidden.size === 0 ? providers : providers.filter((provider) => !hidden.has(provider?.id))

			// 用户要求：LLM 下拉列表优先使用套餐额度，套餐排在上面，按量付费排在下面，从上往下选最容易先选到套餐。
			// 稳定排序：subscription 得分 1，usage 得分 0；相同计费模式保持原有先后相对顺序。
			return [...available].sort((a, b) => {
				const scoreA = billingModeOf(a?.id) === 'subscription' ? 1 : 0
				const scoreB = billingModeOf(b?.id) === 'subscription' ? 1 : 0
