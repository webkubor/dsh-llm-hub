...
import { detectBillingMode, sortProviders } from './sorter.js'
import { AvailabilityStore, isDropDeadHidden, classifyFailure } from './availability-store.js'

export { detectBillingMode, sortProviders } from './sorter.js'
export { AvailabilityStore, isDropDeadHidden, classifyFailure } from './availability-store.js'
...
const BILLING_MODE_PATH = '/api/dsh-llm-hub/billing-mode'
...
/**
 * 路由组状态：`GET /api/dsh-llm-hub/routing/state`。
 *
 * composer 快切 chip 的数据源 —— 返回当前模式（manual/smart）、激活组、每个组
 * 的候选与可用状态、以及 smart 模式最近一次的真实路由判断（lastDecision）。
 */
const ROUTING_STATE_PATH = '/api/dsh-llm-hub/routing/state'
/**
 * 切换模式 / 组：`POST /api/dsh-llm-hub/routing/select`。
 *
 * 请求体 `{ mode?, activeGroup? }`（都可选，至少一个）。写回 settings 的
 * `dsh-llm-hub.routing` 段 —— 状态持久化，重启不丢。
 */
const ROUTING_SELECT_PATH = '/api/dsh-llm-hub/routing/select'
/** 插件自有的 settings 命名空间（1.5.0 起注册）—— routing / warning / aliases 都住这里。 */
const HUB_NS = 'dsh-llm-hub'
/** 模型别名：客户端渲染 model id 时显示短名（典型场景：deepseek-reasoner → 推理）。 */
const ALIASES_PATH = '/api/dsh-llm-hub/aliases'
/**
 * 1.4.0 多 Key 轮换纯函数：支持 string（单 key）或 string[]（多 key round-robin）。
 *
 * @param {string|string[]|undefined} refs - 单 Key 或多 Key 环境变量名数组
 * @param {string} provider - Provider 标识（用于维护独立索引）
 * @param {Map<string, number>} [stateMap] - 轮换游标存储
 * @returns {{ ref: string, index: number, total: number } | undefined}
 */
export function pickRotatedEnv(refs, provider, stateMap) {
	if (typeof refs === 'string' && refs.length > 0) {
		return { ref: refs, index: 0, total: 1 }
	}
	if (Array.isArray(refs)) {
		const list = refs.filter((r) => typeof r === 'string' && r.trim().length > 0)
		if (list.length === 0) return undefined
		const current = stateMap ? (stateMap.get(provider) ?? 0) : 0
		const index = current % list.length
		if (stateMap) stateMap.set(provider, (current + 1) % list.length)
...
/**
 * 智能识别计费模式：'usage'（按量付费）| 'subscription'（套餐）。
 * 优先级：
 * 1. 显式覆盖：profile?.billingMode
 * 2. 定时限额特征（5h/周/月/resetsAt/quota/plan/percent）→ 'subscription'
 * 3. 原厂域名（deepseek.com / bigmodel.cn / openai.com 等）或现金余额 → 'usage'
 * 4. 兜底 → 'usage'


/**
 * 「provider/model」字符串解析（路由组的真源实现；closure 里的
 * `parseRouteEntry` 是同语义副本 —— 测不出 export，所以这里留正源）。
 *
 * provider 严格（字母数字与连字符/下划线），model 放宽（允许 `.` `+` —— DSH
 * 真实 id 会含它们），缺斜杠 / 空段 / 非法字符一律 undefined。
 * @param {string} entry - 配置里的一行。
 * @returns {{ provider: string, model: string } | undefined}
 */
export function parseRouteEntryPure(entry) {
...
/**
 * 把 settings 的 `routing` 段折成路由组列表（1.5.0 智能路由组的真源实现）。
 *
 * 形态：
 *   routing:
 *     mode: smart            # manual | smart（缺省 manual）
 *     activeGroup: company   # 缺省取第一个组
 *     groups:
 *       - id: company
 *         label: 公司
 *         primary: modelgo/gpt-4o
 *         fallbacks: [modelgo/claude-3.5-sonnet]
 *       - id: personal
 *         primary: deepseek-official/deepseek-chat
 *         fallbacks: [minimax/MiniMax-M2.7]
 *
 * 兼容：没有 `groups` 时 flat 的 `primary/fallbacks` 折成一个 id 为
 * `default` 的组 —— 老配置不写 groups 也能继续用。
 *
 * @param {object|undefined|null} routing - settings.routing 段。
 * @returns {{ groups: Array, mode: 'manual'|'smart', activeGroup: string }}
 */
export function parseRoutingGroups(routing) {
	const r = routing !== null && typeof routing === 'object' ? routing : {}
	const mode = r.mode === 'smart' ? 'smart' : 'manual'
	const groups = []
	const seen = new Set()
	for (const raw of Array.isArray(r.groups) ? r.groups : []) {
		...
		const primary = parseRouteEntryPure(raw.primary) ?? null
		const fallbacks = (Array.isArray(raw.fallbacks) ? raw.fallbacks : [])
			.map(parseRouteEntryPure)
			.filter((entry) => entry !== undefined)
		if (primary === null && fallbacks.length === 0) continue
		seen.add(id)
		groups.push({
			id,
	...
	if (groups.length === 0) {
		// legacy 形态：flat primary + fallbacks 折成 default 组。
		const primary = parseRouteEntryPure(r.primary) ?? null
		const fallbacks = (Array.isArray(r.fallbacks) ? r.fallbacks : [])
			.map(parseRouteEntryPure)
			.filter((entry) => entry !== undefined)
		if (primary !== null || fallbacks.length > 0) {
			groups.push({ id: 'default', label: null, primary, fallbacks })
		}
	}
	const declared = typeof r.activeGroup === 'string' ? r.activeGroup.trim() : ''
	const activeGroup = groups.some((group) => group.id === declared) ? declared : (groups[0]?.id ?? '')
	return { groups, mode, activeGroup }
}

/**
 * 智能路由裁决（真源实现，waterfall 与测试共用）。
 *
 * 语义：按组内「primary → fallbacks」顺序，取第一个**未探明不可用**的候选；
 * 额度耗尽（402）/ 网关探测失败都会在可用性表里标 `unavailable`，于是被跳过
 * —— 这就是「按额度自动切换」。全不可用时不动原配置（让请求照常打出去、
 * 错误可见），并给 reason。
 *
 * @param {Array<{ provider: string, model: string }>} entries - 组内候选，primary 在前。
 * @param {(provider: string) => string} stateFor - provider 级可用状态：'available' | 'unknown' | 'unavailable'。
 * @param {{ provider: string, model: string } | null} active - 本次请求机器本来要用的配置。
 * @returns {{ to: { provider: string, model: string } | null, reason: string | null, switched: boolean }}
 */
export function pickSmartRoute(entries, stateFor, active) {
	const list = Array.isArray(entries) ? entries : []
	if (list.length === 0) {
		return { to: null, reason: '当前组没有可用候选 —— 去设置里加 primary / fallbacks', switched: false }
	}
	const winner = list.find((entry) => stateFor(entry.provider) !== 'unavailable')
	if (winner === undefined) {
		return { to: null, reason: '主力与 fallbacks 全部不可用 —— 充值或换一个再说', switched: false }
	}
	const activeKey = active === null || active === undefined ? null : `${active.provider}/${active.model}`
	const winnerKey = `${winner.provider}/${winner.model}`
	if (winnerKey === activeKey) {
		return { to: null, reason: null, switched: false }
	}
	return { to: { provider: winner.provider, model: winner.model }, reason: null, switched: true }
}
...
export function apply(ctx) {
	...
	// ── 插件自有 settings 命名空间（dsh-llm-hub）────────────────────────────
	//
	// 1.5.0 起：routing / warning / aliases 的真源从 `llm-deepseek` 段迁到独立
	// 的 `dsh-llm-hub` 段 —— README 文档里写的顶层 `dsh-llm-hub:` 键此前**从未
	// 被读到过**（没注册过命名空间，settings.get 会抛），这是 1.1.0 起的历史
	// 缺陷，这里一起修掉。
	//
	// schema 尽量懒加载 `@deepseek-ai/schemastery`；失败时退回 `Object` 构造器
	// （schemastery 接受原生构造器作为 schema）。注册失败不 fatal：配置读取退
	// 回 legacy 路径，智能路由降级为「只建议」。
	...
	ctx.inject(['settings'], (settingsCtx) => {
		void (async () => {
			...
			try {
				...
				const z = (mod && typeof mod === 'object' ? mod.default ?? mod : mod)
				const groupItem = z.object({ id: z.string().required(), label: z.string(), primary: z.string(), fallbacks: z.array(z.string()) })
				schema = z.object({
					routing: z.object({
						mode: z.union(['manual', 'smart']).default('manual'),
						activeGroup: z.string().default(''),
						groups: z.array(groupItem).default([])
					}).default({}),
					warning: z.object({ cashCNY: z.number().default(10), planPercent: z.number().default(10) }).default({}),
					aliases: z.dict(z.string()).default({})
				})
			} catch {
				// 零依赖环境：schemastery 没解析到 —— 用原生构造器兜底。
				schema = Object
			}
			try {
				settingsCtx.settings.register(HUB_NS, schema)
				hubRegistered = true
			} catch (error) {
				ctx.logger.warn(`dsh-llm-hub: settings ns "dsh-llm-hub" register failed: ${error?.message ?? error}`)
			}
		})()
	})

	/**
	 * 读 `dsh-llm-hub` 设置段（1.5.0 起的真源）。
	 * @returns 已解析设置段；未注册 / 未配置时为 `undefined`。
	 */
	const hubSectionOf = () => {
		const settings = ctx.get('settings')
		if (settings === undefined) return undefined
		try {
			const value = settings.get(HUB_NS)
	...
	const warningThresholds = () => {
		// 真源是 dsh-llm-hub 段（1.5.0 注册）；legacy llm-deepseek 段只补缺。
		const hub = hubSectionOf() ?? {}
		const legacy = sectionOf() ?? {}
		const merged = { ...(legacy.warning !== null && typeof legacy.warning === 'object' ? legacy.warning : {}), ...(hub.warning !== null && typeof hub.warning === 'object' ? hub.warning : {}) }
	 ...
	 *
	/**
	 * 多 Key 自动轮换索引 —— per provider 维护独立的自增游标。
	 ...
	 *   2) section.apiKeyEnv 是 string[] → 按 provider 轮换取下一个 ref，
	 *      再走 credentials 服务或 process.env 解析密钥
	 *   3) section.apiKeyEnv 是 string → 原单 key 路径（行为完全不变）
	 *
	 * 返 ref 不变 —— 客户端拿到的是凭据引用；下游诊断（探针 / 余额路由）
	 * 能继续看到当前用的是哪个 env。
	...
	const connectionFacts = async (request = {}) => {
		...
		const picked = pickRotatedEnv(section.apiKeyEnv, providerKey, keyRotationIndex)
		const ref = picked === undefined
			? (label(request.apiKeyEnv, section.apiKeyEnv) ?? DEFAULT_API_KEY_ENV)
			: picked.ref
		let apiKey
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (request, response) => {
			...
			try {
				const section = sectionOf() ?? {}
				const refs = Array.isArray(section.apiKeyEnv)
					? section.apiKeyEnv.filter((r) => typeof r === 'string' && r.trim().length > 0)
					: (typeof section.apiKeyEnv === 'string' && section.apiKeyEnv.trim().length > 0
						? [section.apiKeyEnv.trim()]
						: [DEFAULT_API_KEY_ENV])
				const credentials = ctx.get('credentials')
				const keys = await Promise.all(refs.map(async (ref) => {
					let apiKey
					if (credentials !== undefined) {
						const hit = await credentials.resolve(ref)
						apiKey = label(hit?.value)
					...
					if (apiKey === undefined) apiKey = label(process.env[ref])
					if (!apiKey) return { ref, keyMasked: '未配置', isAvailable: false, total: '0.00', error: '未找到密钥', ok: false }
					try {
						const raw = await getJson('/user/balance', { apiKey })
						const shaped = shapeBalance(raw)
						const first = shaped.balances?.[0]
						return {
							ref,
							keyMasked: maskKey(apiKey),
							isAvailable: shaped.isAvailable,
							total: first?.total ?? '0.00',
							currency: first?.currency ?? 'CNY',
							ok: true
						}
					} catch (e) {
						return { ref, keyMasked: maskKey(apiKey), isAvailable: false, total: '0.00', error: e instanceof Error ? e.message : String(e), ok: false }
					}
				}))
				...
				send(200, {
					...
					apiKey: apiKey ?? null,
					keyMasked: maskKey(apiKey),
					billingMode: section.billingMode || 'usage',
					keys,
					...payload
				})
			} catch (error) {
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (request, response) => {
			...
			if (!sameOrigin(request)) {
				send(403, { ok: false, error: 'cross-origin requests are refused' })
				return
			}
			try {
				const collected = []
				for await (const chunk of request) collected.push(chunk)
				const body = JSON.parse(Buffer.concat(collected).toString('utf8'))
				const provider = label(body?.provider)
				const mode = label(body?.mode)
				if (mode !== 'usage' && mode !== 'subscription') {
					send(400, { ok: false, error: 'mode must be "usage" or "subscription"' })
					return
				}
				const settings = ctx.get('settings')
				if (!settings) {
					send(500, { ok: false, error: 'settings service not available' })
					return
				}
				if (provider === 'deepseek' || provider === 'llm-deepseek' || provider === 'deepseek-official') {
					await settings.update('llm-deepseek', { billingMode: mode })
				} else if (provider) {
					const profile = piaiProfileOf(provider) ?? {}
					await settings.update(PIAI_NS, { providers: { [provider]: { ...profile, billingMode: mode } } })
				}
				send(200, { ok: true, provider, mode })
			} catch (error) {
				send(500, { ok: false, error: error instanceof Error ? error.message : String(error) })
			}
		}
	}), 'dsh-llm-hub: set billing mode route')
	//#endregion
	...
	const piaiStatus = async (provider) => {
		...
		return {
			...
			billingMode: label(profile.billingMode) ?? null,
		...
		minimax: {
			...
			shape: (body) => {
				...
				for (const row of rows) {
					...
					let resetsAt = undefined
					const rawTime = row.current_interval_end_time ?? row.end_time ?? row.reset_time
					if (typeof rawTime === 'number' && rawTime > 0) {
						const date = new Date(rawTime > 1e11 ? rawTime : rawTime * 1000)
						if (!isNaN(date.getTime())) {
							resetsAt = `${String(date.getHours()).padStart(2, '0')}:${String(date.getMinutes()).padStart(2, '0')}`
						}
					} else if (typeof rawTime === 'string' && rawTime.trim() !== '') {
						resetsAt = rawTime.trim()
					}
					items.push({
		...
		zhipu: {
			...
			shape: (body) => {
				...
				for (const lim of limits) {
					...
					if (percent === undefined) continue
					let resetsAt = undefined
					const rawTime = lim.reset_time ?? lim.next_reset_time ?? body?.data?.expire_time
					if (typeof rawTime === 'number' && rawTime > 0) {
						const date = new Date(rawTime > 1e11 ? rawTime : rawTime * 1000)
						if (!isNaN(date.getTime())) {
							resetsAt = `${String(date.getHours()).padStart(2, '0')}:${String(date.getMinutes()).padStart(2, '0')}`
					...
					items.push({ label: tier, percent, resetsAt })
				}
	...
	/** 可用性与健康状态机实例 */
	const availabilityStore = new AvailabilityStore({ runtimeTtlMs: RUNTIME_TTL_MS })
	/** provider -> 探针判定（凭据 / 探测 / 余额）。 */
	const baseVerdicts = availabilityStore.baseVerdicts
	/** provider -> 最近一次运行期失败判定（真实请求的鉴权/欠费错误）。 */
	const runtimeMarks = availabilityStore.runtimeMarks
	/** provider -> 最近一次真实请求成功的时刻。 */
	const successAt = availabilityStore.successAt
	/** 全量重探的去重句柄。 */
	let probeAll = null
	...
	const currentAvailability = () => availabilityStore.currentAvailability()

	/** 当前该从下拉里摘掉的 provider 集合。 */
	const hiddenProviders = () => availabilityStore.hiddenProviders()
	...
	const installProviderFilter = () => {
		...
		const billingModeOf = (id) => {
			if (!id || typeof id !== 'string') return 'usage'
			const cleanId = id.replace(/^pi-ai:/, '')
			if (cleanId === 'deepseek-official' || cleanId === 'deepseek' || cleanId === 'llm-deepseek') {
				const section = sectionOf()
				if (section?.billingMode === 'usage' || section?.billingMode === 'subscription') {
					return section.billingMode
				}
				return 'usage'
			}
			const profile = piaiProfileOf(cleanId) || piaiProfileOf(id)
			if (profile?.billingMode === 'usage' || profile?.billingMode === 'subscription') {
				return profile.billingMode
			}
			return detectBillingMode(profile || { provider: cleanId })
		}

		const filtered = function (...args) {
			const providers = original.apply(this, args)
			if (!Array.isArray(providers)) return providers
			const hidden = availabilityStore.hiddenProviders()
			// 用户要求：LLM 下拉列表优先使用套餐额度，套餐排在上面，按量付费排在下面，从上往下选最容易先选到套餐。
			return sortProviders(providers, (id) => ({
				available: !hidden.has(id),
				billingMode: billingModeOf(id)
	...
	/**
	 * 解析「provider/model」字符串 —— 委托给模块级真源 `parseRouteEntryPure`。
	 * @param entry - 配置里的一行。
	 * @returns 解析后的 { provider, model }，无效时为 `undefined`。
	 */
	const parseRouteEntry = (entry) => parseRouteEntryPure(entry)

	/**
	 * 合并读 settings 的 `routing` 段：真源 `dsh-llm-hub`（1.5.0 注册），
	 * legacy `llm-deepseek` 段只补缺 —— 用户科研把配置写在新旧两处也能读。
	 * @returns 合并后的 routing 对象（可能为空对象）。
	 */
	const routingObjOf = () => {
		const hub = hubSectionOf() ?? {}
		const legacy = sectionOf() ?? {}
		const hubRouting = hub.routing !== null && typeof hub.routing === 'object' ? hub.routing : {}
		const legacyRouting = legacy.routing !== null && typeof legacy.routing === 'object' ? legacy.routing : {}
		return { ...legacyRouting, ...hubRouting }
	}

	/**
	 * 读路由组（真源实现 `parseRoutingGroups`，settings 惰性重读）。
	 * @returns {{ groups: Array, mode: 'manual'|'smart', activeGroup: string }}
	 */
	const routingGroupsOf = () => parseRoutingGroups(routingObjOf())

	/**
	 * legacy 候选列表（老 resolve 语义保留）：当前激活组展开成
	 * primary → fallbacks 顺序数组。
	 * @returns 配置好的顺序数组，无效项过滤掉。空数组表示未配置。
	 */
	const routingCandidatesOf = (groupId) => {
		const state = routingGroupsOf()
		const group = state.groups.find((candidate) => candidate.id === groupId) ?? state.groups[0]
		if (group === undefined) return []
		const out = []
		if (group.primary !== null) out.push(group.primary)
		for (const fallback of group.fallbacks) out.push(fallback)
		return out
	}
	...
	/**
	 * 路由解析：给客户端返回「该用哪个 + 原因」（1.5.0 起按组解析）。
	 * @param active - 客户端 POST 进来的当前已激活 (provider, model)。
	 * @param groupId - 指定组 id；缺省用当前激活组。
	 * @returns 路由建议载荷（附 mode / activeGroup / groups 摘要）。
	 */
	const resolveRoute = (active, groupId) => {
		const state = routingGroupsOf()
		const candidates = routingCandidatesOf(typeof groupId === 'string' && groupId !== '' ? groupId : undefined)
		const activeGroup = state.groups.some((group) => group.id === groupId) ? groupId : state.activeGroup
		...
		return {
			...
			groups: state.groups.map((group) => ({
				id: group.id,
				label: group.label,
				candidateCount: (group.primary !== null ? 1 : 0) + group.fallbacks.length
			})),
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (request, response) => {
			...
			sendJson(response, 200, resolveRoute(payload?.active, typeof payload?.group === 'string' ? payload.group : undefined))
		}
	}), 'dsh-llm-hub: routing resolve route')

	// ── 智能路由组 + 模式接管（1.5.0）────────────────────────────────────────
	//
	// 上面的 resolveRoute 是「建议」；这一段是「接管」：
	//
	//   · 配置（settings.dsh-llm-hub.routing）：mode = manual|smart、activeGroup、
	//     groups[] —— 每组 primary + fallbacks[]，「公司」「个人」就是两个组。
	//   · smart 模式：监听 `agent/request` waterfall，每次模型调用拿机器本要用
	//     的 config，按激活组 primary → fallbacks 找第一个未探明不可用的候选，
	//     直接替换 provider/model —— **完全接管**，composer 下拉显示什么不再
	//     决定实际路由（额度耗尽 = 探测 unavailable = 被跳过，这就是「按额度
	//     自动切换」）。waterfall 是官方请求改写点（«Replace the frozen call
	//     configuration»），合法；README 里「不做自动切换」的旧红线只针对
	//     composer slot 本身，这条不碰 slot。
	//   · manual 模式：行为与 1.1.0 完全一致 —— 只建议，不改请求。
	//   · 切组 / 切模式：POST /routing/select 持久化进 settings，重启不丢。
	//
	// 稳定状态：最近一次 smart 判断存 `lastSmartDecision`（内存即可，用于 chip
	// 展示「当前实际路由」）。
	const lastSmartDecision = { at: 0, group: null, from: null, to: null, reason: null }

	/** smart 状态给前端（不带 settings 原文）。 */
	const smartStateOf = () => {
		const state = routingGroupsOf()
		const groups = state.groups.map((group) => {
			const candidates = [
				...(group.primary !== null ? [group.primary] : []),
				...group.fallbacks
			]
			const winner = pickSmartRoute(candidates, (provider) => stateFor(provider, ''), null)
			return {
				id: group.id,
				label: group.label,
				candidates: candidates.map((candidate) => ({
					provider: candidate.provider,
					model: candidate.model,
					state: stateFor(candidate.provider, candidate.model)
				})),
				wouldRoute: winner.to,
		...
		return {
			...
			groups,
			lastDecision: lastSmartDecision.at > 0 ? {
				at: lastSmartDecision.at,
				group: lastSmartDecision.group,
				from: lastSmartDecision.from,
				to: lastSmartDecision.to,
				reason: lastSmartDecision.reason
			} : null
		}
	}

	/** 读 settings 的 routing 段并按需覆写 mode / activeGroup，持久化。 */
	const applyRoutingSelect = async (patch) => {
		const settings = ctx.get('settings')
		if (settings === undefined || hubRegistered !== true) {
			return { ok: false, error: 'settings 命名空间 dsh-llm-hub 尚未就绪 —— 稍后重试' }
		}
		const current = routingObjOf()
		const merged = { ...current }
		if (patch.mode === 'manual' || patch.mode === 'smart') merged.mode = patch.mode
		if (typeof patch.activeGroup === 'string' && patch.activeGroup.trim() !== '') merged.activeGroup = patch.activeGroup.trim()
		try {
			await settings.update(HUB_NS, { routing: merged })
			return { ok: true, ...smartStateOf() }
		} catch (error) {
			return { ok: false, error: `写不进 settings（${error?.message ?? error}）` }
		}
	}

	/** 读一个 POST body（JSON）；非法返 null。 */
	const readPostBody = async (request) => {
		try {
			const collected = []
			for await (const chunk of request) collected.push(chunk)
			return JSON.parse(Buffer.concat(collected).toString('utf8'))
		} catch {
			return null
		}
	}
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: (request, response) => {
			...
			ensureAvailabilityFresh()
			sendJson(response, 200, smartStateOf())
		}
	}), 'dsh-llm-hub: routing state route')

	ctx.effect(() => ctx.webServer.register({
		kind: 'exact',
		path: ROUTING_SELECT_PATH,
		handler: async (request, response) => {
			if (request.method !== 'POST') {
				response.writeHead(405, { allow: 'POST' })
				response.end()
			...
			const result = await applyRoutingSelect({
				mode: payload?.mode,
				activeGroup: payload?.activeGroup
			})
			sendJson(response, result.ok === true ? 200 : 503, result)
		}
	}), 'dsh-llm-hub: routing select route')

	// smart 模式的请求接管。global: true —— 根上下文的监听要覆盖所有 agent
	// （含子代理），否则「自动按额度切换」只对当前会话生效。
	// 任何异常都吞掉并返回 next() 的原 config：路由接管必须 fail-open，不能
	// 因为分组配置写错了一行让会话发不出请求。
	ctx.on('agent/request', async (payload, next) => {
		...
		try {
			...
			const state = routingGroupsOf()
			if (state.mode !== 'smart' || state.groups.length === 0) return config
			const group = state.groups.find((candidate) => candidate.id === state.activeGroup) ?? state.groups[0]
			const candidates = [
				...(group.primary !== null ? [group.primary] : []),
				...group.fallbacks
			]
			const active = { provider: config.provider, model: config.model }
			const decision = pickSmartRoute(candidates, (provider) => stateFor(provider, ''), active)
			lastSmartDecision.at = Date.now()
			lastSmartDecision.group = group.id
			lastSmartDecision.from = active
			lastSmartDecision.to = decision.to
			lastSmartDecision.reason = decision.reason
			if (decision.switched !== true || decision.to === null) return config
			return { ...config, provider: decision.to.provider, model: decision.to.model }
		} catch (error) {
			ctx.logger.warn(`dsh-llm-hub: smart routing intercept failed, keeping original config: ${error?.message ?? error}`)
			return config
		}
	}, { global: true })
	...
	const aliasesOf = () => {
		...
		const legacy = sectionOf() ?? {}
		const raw = {
			...(legacy.aliases !== null && typeof legacy.aliases === 'object' ? legacy.aliases : {}),
			...(hub.aliases !== null && typeof hub.aliases === 'object' ? hub.aliases : {})
		}
		const out = {}
