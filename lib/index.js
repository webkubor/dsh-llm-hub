...
export function apply(ctx) {
		...
		minimax: {
			...
			shape: (body) => {
				...
				for (const row of rows) {
					...
					const percent = capacity(row.current_interval_remaining_percent)
					if (percent === undefined) continue
					items.push({
						label: label(row.model_name) ?? 'plan',
						percent,
						weeklyPercent: capacity(row.current_weekly_remaining_percent)
					})
				}
				return { items }
			}
		...
		zhipu: {
			...
			shape: (body) => {
				...
				for (const lim of limits) {
					...
					const percent = capacity(lim.percentage)
					if (percent === undefined) continue
	...
	const piaiBalance = async (provider) => {
		...
		if (apiKey === undefined) {
			return { ok: true, provider, kind: adapter.kind, available: false, reason: `未配置 ${ref ?? 'API'} key` }
		}
		...
		try {
			...
			return { ok: true, provider, kind: adapter.kind, available: false, reason: `could not reach ${url}: ${error instanceof Error ? error.message : String(error)}` }
		}
		if (!response.ok) {
			return { ok: true, provider, kind: adapter.kind, available: false, reason: `${url} answered ${response.status}` }
		}
		const text = await readBounded(response, url)
		let body
		try {
			...
			return { ok: true, provider, kind: adapter.kind, available: false, reason: `${url} did not answer with JSON` }
		}
